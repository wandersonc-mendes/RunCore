...
export default function ProfilePanel({ onClose }) {
  ...
  async function save() { setSaving(true); setMessage(""); try { await saveStudentProfile(profile); setMessage("Perfil salvo com sucesso."); } catch (error) { setMessage(error.message); } finally { setSaving(false); } }

  return <main className="profile-page"><header className="student-header"><div className="brand"><span className="brand-logo"><img src="/logo-horizontal.png?v=2" alt="RunCore" /></span><div><h1>Meu perfil</h1><p>Complete seus dados para o acompanhamento</p></div></div><button className="btn-ghost" onClick={onClose}>Voltar</button></header><section className="profile-content"><div className="profile-tabs"><button className={tab === "personal" ? "active" : ""} onClick={() => setTab("personal")}>Dados pessoais</button><button className={tab === "parq" ? "active" : ""} onClick={() => setTab("parq")}>PAR-Q e saúde</button><button className={tab === "training" ? "active" : ""} onClick={() => setTab("training")}>Dados de treino</button></div>
